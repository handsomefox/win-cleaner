...
func showEmptyRootSelect(opts Options, a fyne.App, w fyne.Window, safeClose func(error)) {
	roots := DefaultEmptyFolderRoots()
	if len(roots) == 0 {
...
func showEmptyRootSelectWithRoots(roots []EmptyFolderRoot, opts Options, a fyne.App, w fyne.Window, safeClose func(error)) {
	selectedLabel := widget.NewLabel("")
	updateSummary := func() {
		selected := 0
...
func showEmptyScan(roots []EmptyFolderRoot, opts Options, a fyne.App, w fyne.Window, safeClose func(error)) {
	...
	go func() {
		plan := BuildEmptyFolderPlan(roots, func(u ProgressUpdate) {
			a.Driver().DoFromGoroutine(func() {
				if u.Total > 0 {
					progress.SetValue(float64(u.Current) / float64(u.Total))
				}
...
func showEmptySelect(plan *EmptyFolderPlan, opts Options, a fyne.App, w fyne.Window, safeClose func(error)) {
	filterEntry := widget.NewEntry()
	...
	errText := emptyScanErrorText(plan.Errs)
	if errText == "" {
		errText = fmt.Sprintf("Found %d empty folders.", len(plan.Folders))
	}
...
func showEmptyDelete(plan *EmptyFolderPlan, opts Options, a fyne.App, w fyne.Window, safeClose func(error)) {
	...
	go func() {
		result := ExecuteEmptyFolderPlan(plan, func(u ProgressUpdate) {
			a.Driver().DoFromGoroutine(func() {
				if u.Total > 0 {
					progress.SetValue(float64(u.Current) / float64(u.Total))
...
func showEmptyResults(result *EmptyFolderResult, opts Options, a fyne.App, w fyne.Window, safeClose func(error)) {
...
func emptyScanErrorText(errs []PathError) string {
	if len(errs) == 0 {
		return ""
	}
